...
Trata-se da "Versão 2" do sistema bancário proposto no Bootcamp.
...
Aprimorar a estrutura e a eficiência do sistema, refatorando o código implementando as operações de depósito, saque e extrato em funções específicas.
...
**NOVO!** A função de depósito deve receber argumentos apenas por posição (positional only). 
...
**NOVO!** A função de saque deve receber os argumentos apenas por nome (keyword only).
...
**NOVO!** A função de extrato deve receber os argumentos por posição e nome (positional only e keyword only). Argumento posicional: saldo, argumento nomeado: extrato.
